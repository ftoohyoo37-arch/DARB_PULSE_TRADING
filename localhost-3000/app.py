...
SAMPLE_DEPOSITS_DIR = ROOT / "sample-deposits"
if not SAMPLE_DEPOSITS_DIR.is_dir():
    raise ImportError(
        f"Could not find sample-deposits pipeline package at {SAMPLE_DEPOSITS_DIR}. "
        "Ensure you are running the app from the repository root and that the "
        "sample-deposits folder is present."
    )
if str(SAMPLE_DEPOSITS_DIR) not in sys.path:
    sys.path.insert(0, str(SAMPLE_DEPOSITS_DIR))
